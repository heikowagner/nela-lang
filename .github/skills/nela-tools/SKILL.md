...
> **Architecture note (v0.2):** LLMs read and write NELA-S (surface language).
...
**Purpose:** Evaluate NELA-S programs (typed expression DAG JSON). This is the primary
tool an LLM uses to run and verify programs it has written.
...
def run_program(prog: dict, fn_name: str, *arg_values) -> Any:
    ...
    prog:       parsed JSON dict  {nela_version, program, defs: [...]}
