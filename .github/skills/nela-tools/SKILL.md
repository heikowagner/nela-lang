...
> **Architecture note (v0.3):** LLMs read and write NELA-S in S-expression syntax (.nela files).
...
**Purpose:** Evaluate NELA-S programs (S-expression `.nela` files). Parse with `nela_parser.parse_file(path)` or `nela_parser.parse_program(text)`, then run with `run_program`. This is the primary tool an LLM uses to run and verify programs it has written.
